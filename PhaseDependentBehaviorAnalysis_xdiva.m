...
out.EventEncCondAtRet       = behav_out.Trials.EncCondAtRet; % e
out.EncStimAtRet            = behav_out.Trials.EncStimIDAtRet;
out.HitMissTrialIDs         = nan(nSubjs,2,450);
out.ValidTrialIDs           = nan(nSubjs,450);
...
for ss = 1:out.nSubjs
    ...
    miss = behav_out.retSubj{ss}.Misses; % miss IDs @ retrieval
    out.HitMissTrialIDs(ss,1,:) = hits;
    out.HitMissTrialIDs(ss,2,:) = miss;
    out.ValidTrialIDs(ss,:)     = squeeze(hits|miss);
    
    % Get true phase for Hits and Misses
    out.HitMissPhases{ss,1} = out.EventPhase(ss,behav_out.EncRet.EncHitTrialIDs{ss});    
    out.HitMissPhases{ss,2} = out.EventPhase(ss,behav_out.EncRet.EncMissTrialIDs{ss});    
    
    % Get phase condition for Hits and Misses
    out.HitMissPhaseCond{ss,1} = out.EventPhaseCond(ss,behav_out.EncRet.EncHitTrialIDs{ss});    
    out.HitMissPhaseCond{ss,2} = out.EventPhaseCond(ss,behav_out.EncRet.EncMissTrialIDs{ss});    
    
    % Get Confidence and assign sign based on hit/miss
    Confidence                  = behav_out.retSubj{ss}.Confidence;
    Confidence(miss)            = Confidence(miss)*-1;
